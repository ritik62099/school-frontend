...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      // ✅ Fix: Include all classes from both sources
...
const handleUpdateSubject = async () => {
  ...
  return (
    <div style={{ padding: '1.5rem', fontFamily: 'Poppins, sans-serif', maxWidth: '1000px', margin: '0 auto' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h2 style={{ color: '#2c3e50', margin: 0 }}>Manage Classes & Subjects</h2>
        <button
          onClick={() => navigate(-1)}
          style={{
            padding: '0.5rem 1rem',
      ...
      {/* Add Class */}
      {/* Add Class */}
<div style={{ marginBottom: '2rem', padding: '1rem', background: '#f0f9ff', borderRadius: '10px' }}>
  <h3 style={{ margin: '0 0 0.8rem' }}>Add / Delete Class</h3>
  <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.8rem' }}>
    ...
    />
    <button onClick={handleAddClass} style={{ padding: '0.6rem 1rem', background: '#2ecc71', color: 'white', border: 'none', borderRadius: '6px', fontWeight: '600' }}>
      Add
    </button>
  </div>
...
{classes.length > 0 && (
  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.8rem' }}>
    <select
      ...
      onChange={(e) => {
        ...
        setUpdateClassTo(e.target.value); // pre-fill
      }}
      style={{ padding: '0.6rem', borderRadius: '6px', border: '1px solid #ccc', flex: 1 }}
    >
  ...
  <div style={{ marginTop: '0.8rem', fontSize: '0.9rem', color: '#555' }}>
    Current Classes: {classes.length ? classes.join(', ') : 'None'}
      ...
      {/* Add Subject */}
      {/* Add Subject */}
<div style={{ marginBottom: '2rem', padding: '1rem', background: '#f0fff4', borderRadius: '10px' }}>
  <h3 style={{ margin: '0 0 0.8rem' }}>Add / Delete Subject</h3>
  <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.8rem' }}>
    <input
      value={newSubject}
      ...
      style={{ padding: '0.6rem', flex: 1, borderRadius: '6px', border: '1px solid #ccc' }}
    />
    <button onClick={handleAddSubject} style={{ padding: '0.6rem 1rem', background: '#2ecc71', color: 'white', border: 'none', borderRadius: '6px', fontWeight: '600' }}>
      Add
    </button>
  </div>

  {/* Delete Subject Dropdown */}
  {subjects.length > 0 && (
    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.8rem' }}>
      <select
...
{subjects.length > 0 && (
  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.8rem' }}>
    <select
      ...
      onChange={(e) => {
        ...
        setUpdateSubjectTo(e.target.value); // pre-fill
      }}
      style={{ padding: '0.6rem', borderRadius: '6px', border: '1px solid #ccc', flex: 1 }}
    >
  ...
  <div style={{ marginTop: '0.8rem', fontSize: '0.9rem', color: '#555' }}>
    Available Subjects: {subjects.length ? subjects.join(', ') : 'None'}
      ...
      {/* Editable Assignment Section */}
      <div style={{ marginBottom: '3rem' }}>
        <h3 style={{ marginBottom: '1rem', color: '#2c3e50' }}>Assign Subjects to Classes</h3>
        {classes.length === 0 ? (
          <p>No classes available.</p>
        ) : (
          classes.map(cls => (
            <div key={cls} style={{ background: '#fff', padding: '1.2rem', margin: '1rem 0', borderRadius: '10px', boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }}>
              <h4 style={{ margin: '0 0 1rem', color: '#2980b9' }}>{cls}</h4>
              <select
  multiple
  value={classSubjects[cls] || []}
  onChange={(e) => handleSubjectChange(cls, handleMultiSelectChange(e))}
  style={{ width: '100%', minHeight: '100px', padding: '0.5rem', borderRadius: '6px', border: '1px solid #ddd' }}
>
  {subjects.map(sub => (
    <option key={`${cls}-${sub}`} value={sub}>{sub}</option>
  ))}
</select>

{/* 👇 Live Preview of selected subjects */}
<div style={{ marginTop: '0.6rem', color: '#34495e' }}>
  <strong>Selected Subjects:</strong>{' '}
  {classSubjects[cls]?.length > 0 ? (
    <span>{classSubjects[cls].join(', ')}</span>
  ) : (
    <span style={{ color: '#95a5a6', fontStyle: 'italic' }}>No subjects selected</span>
  )}
</div>
...
>
  {savingClass === cls ? 'Saving...' : 'Save Subjects'}
      ...
      {/* ✅ READ-ONLY SUMMARY: "Kon si class mein kon se subjects hain" */}
      <div style={{ paddingTop: '2rem', borderTop: '2px solid #e2e8f0' }}>
        <h3 style={{ color: '#2c3e50', marginBottom: '1.2rem' }}>Class-wise Subjects Overview</h3>
        {classes.length === 0 ? (
