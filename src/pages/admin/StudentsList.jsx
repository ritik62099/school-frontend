...
import { useNavigate } from "react-router-dom";
import { useStudents } from "../../hooks/useStudents";
import { endpoints } from "../../config/api"; // ✅ ADD THIS
...
const StudentsList = () => {
  ...
  const handlePromote = async (studentId) => {
    ...
    try {
      ...
      formData.append("class", promoteClass);
      formData.append("rollNo", ""); // 🔹 roll number ko empty karna

      const res = await fetch(`${endpoints.students.list}/${studentId}`, {
        method: "PUT",
        headers: {
      ...
      // Agar aapke useStudents hook me refetch hai to use karlo
      // refetch();
