...
const AttendanceOverview = () => {
  ...
  // Manual Search Button: load attendance for all students
  const searchAttendance = async () => {
    ...
    try {
      ...
      setClassReports(results.filter(Boolean));
    } catch (err) {
  ...
  // Show daily report on click
  const showDailyReport = (item) => {
    setStudentInfo(item.student);
    setDailyReport(item.report);
  };

  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>Class Attendance Overview</h2>
      ...
      <div style={styles.form}>
        ...
        <button onClick={searchAttendance} style={styles.button}>
      ...
      {classReports.length > 0 && (
        <table style={styles.table}>
          ...
          <tbody>
            {classReports.map((item, i) => {
              const present = item.report.filter((r) => r.present).length;
              const total = item.report.length;
              ...
              return (
                <tr
                  ...
                  onClick={() => showDailyReport(item)}
                >
                  <td style={styles.td}>{item.student.name}</td>
                  <td style={styles.td}>{present}</td>
                  <td style={styles.td}>{absent}</td>
      ...
      {dailyReport.length > 0 && (
        <>
          <div style={styles.studentCard}>
            <strong>{studentInfo.name}</strong> — Class:{" "}
            {studentInfo.class}, Roll: {studentInfo.rollNo}
          </div>
...
const styles = {
  ...
  td: { padding: "10px", borderBottom: "1px solid #eee", textAlign: "center" },
  studentCard: {
    background: "#eef2ff",
    padding: "12px",
    borderRadius: "8px",
